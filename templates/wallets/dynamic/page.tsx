...
export default function Home() {
  ...
  // Get chain-specific branding from environment
  const chainName = process.env.NEXT_PUBLIC_CHAIN_NAME || "Blockchain";

  // Dynamic context for auth flow
  ...
  const { blink, isLoading } = useBlink({
    url: `evm-action:${typeof window !== 'undefined' ? window.location.origin : 'http://localhost:3000'}/api/actions/tip-blink`,
  });
    ...
    <main className="grid grid-cols-[2fr_3fr] h-[calc(100vh-64px)]">
      <div className="col-span-1 p-8 pr-16 overflow-y-auto">
        <h1 className="text-[40px] mb-3 font-bold leading-[1]">
          {chainName} Blinks Starter Template
        </h1>
